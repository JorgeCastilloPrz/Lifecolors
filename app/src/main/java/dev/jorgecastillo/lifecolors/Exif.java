...
public final class Exif {
    ...
    public Location getLocation() {
        ...
        if (speed != 0) {
            switch (speedRef) {
                case MILES_PER_HOUR:
                    speed = Exif.Speed.fromMilesPerHour(speed).toMetersPerSecond();
                    break;
                case KNOTS:
                    speed = Exif.Speed.fromKnots(speed).toMetersPerSecond();
                    break;
                case KILOMETERS_PER_HOUR:
                    // fall through
                default:
                    speed = Exif.Speed.fromKilometersPerHour(speed).toMetersPerSecond();
                    break;
            }
    ...
    private static final class Speed {
        static Exif.Speed.Converter fromKilometersPerHour(double kph) {
            return new Exif.Speed.Converter(kph * 0.621371);
        }

        static Exif.Speed.Converter fromMetersPerSecond(double mps) {
            return new Exif.Speed.Converter(mps * 2.23694);
        }

        static Exif.Speed.Converter fromMilesPerHour(double mph) {
            return new Exif.Speed.Converter(mph);
        }

        static Exif.Speed.Converter fromKnots(double knots) {
            return new Exif.Speed.Converter(knots * 1.15078);
        }
